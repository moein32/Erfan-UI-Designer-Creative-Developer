...
export const InteractiveApproach: React.FC = () => {
  ...
  const colorTokens = [
    { id: 'ink-primary', name: '--token-ink', hex: '#F5F5F7', label: 'Primary Ink' },
    { id: 'glass-specular', name: '--token-glass-highlight', hex: '#FFFFFF', label: 'Specular Shine' },
    { id: 'neutral-silver', name: '--token-surface-subtle', hex: '#1C1C1F', label: 'Dark Surface' },
    { id: 'accent-emerald', name: '--token-telemetry-active', hex: '#34D399', label: 'Living State' },
  ];
  ...
  return (
    ...
    >
      ...
      <TypographicWatermark text="SYSTEM" position="center" opacity="opacity-[0.02]" />
      ...
      >
        ...
        <div>
          <div className="flex items-center justify-between gap-4 pb-6 border-b border-white/10">
            <div className="flex items-center gap-3">
              <span className="w-2.5 h-2.5 rounded-full bg-[#F5F5F7] shadow-[0_0_8px_rgba(245,245,247,0.6)]" />
            ...
            {/* Interactive Step Switcher Tabs — Dark Liquid Glass */}
            <div className="flex items-center gap-1.5 p-1 rounded-full glass-subtle border border-white/10">
              {steps.map((step, idx) => (
                <button
                  key={step.phase}
        ...
        <div className="my-auto py-8 grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-14 items-center">
          ...
          <div className="lg:col-span-6 space-y-6">
            ...
            {/* Step Phase Badge */}
            <div className="inline-flex items-center gap-2.5 px-3.5 py-1.5 rounded-full glass-subtle border border-white/10">
              <div className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
              <span className="font-mono text-xs font-bold tracking-widest text-[#F5F5F7] uppercase">
                {formatNumber(currentStep.number)} · {currentStep.phase}
              </span>
            </div>

            {/* Step Title */}
            <h3 className="text-2xl sm:text-4xl md:text-5xl font-black text-[#F5F5F7] tracking-tight leading-tight transition-all duration-300">
              {currentStep.title}
            </h3>

            {/* Step Description */}
            <p className="text-base sm:text-lg leading-relaxed text-[#A1A1AA] font-normal transition-all duration-300">
              {currentStep.desc}
            </p>
            ...
            <div className="space-y-2.5 pt-2">
              {currentStep.bulletPoints.map((bp) => (
                <div key={bp} className="flex items-center gap-3 text-xs sm:text-sm font-mono text-[#F5F5F7]">
            ...
            <div className="flex items-center gap-3 pt-4">
              <button
                ...
                className="w-10 h-10 rounded-full glass-subtle border border-white/10 flex items-center justify-center text-[#F5F5F7] disabled:opacity-30 disabled:cursor-not-allowed hover:bg-white hover:text-[#070709] transition-all cursor-pointer shadow-md"
                onMouseEnter={() => setCursor({ type: 'button' })}
                onMouseLeave={resetCursor}
              >
          ...
          {/* Central Visual Object: Authentic Dark Liquid Glass Design System Interface */}
          <div className="lg:col-span-6 flex justify-center">
            <div
              ref={hudRef}
              className="w-full max-w-[480px] rounded-[32px] glass-medium border border-white/12 p-6 sm:p-8 shadow-2xl space-y-6 relative overflow-hidden group"
            >
              {/* Top Specular Shimmer Reflection */}
              <div className="absolute inset-x-0 top-0 h-32 bg-gradient-to-b from-white/15 via-white/5 to-transparent pointer-events-none" />

              {/* Interface Header */}
              <div className="flex items-center justify-between gap-4 pb-4 border-b border-white/10 relative z-10">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-xl bg-white text-[#070709] flex items-center justify-center shadow-md font-bold">
                    {stepIcons[activeStepIndex] || <Sparkles size={16} />}
                  </div>
                  <div>
                    <span className="block font-mono text-xs font-bold text-[#F5F5F7] uppercase tracking-wider">
                ...
                <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20">
                  ...
                  <span className="font-mono text-[10px] font-bold text-emerald-400">60 FPS</span>
                </div>
              </div>
              ...
              <div className="relative z-10 space-y-4">
                ...
                {activeStepIndex === 0 && (
                  <div className="p-5 rounded-2xl glass-subtle border border-white/10 space-y-4">
                    <div className="flex items-center justify-between text-[11px] font-mono text-[#A1A1AA]">
                      ...
                      <span className="text-[#F5F5F7] font-bold">RATIO 1.333 (4:3)</span>
                    </div>

                    <div className="space-y-2 border-y border-white/8 py-3">
                      <div className="flex items-baseline justify-between">
                        <span className="text-xl font-bold tracking-tight text-[#F5F5F7]">Display Headline</span>
                        <span className="font-mono text-[10px] text-[#71717A]">48px / -0.03em</span>
                      </div>
                      <div className="flex items-baseline justify-between">
                        <span className="text-sm font-medium text-[#A1A1AA]">Editorial Body Cadence</span>
                ...
                {activeStepIndex === 1 && (
                  <div className="p-5 rounded-2xl glass-subtle border border-white/10 space-y-4">
                    <div className="flex items-center justify-between text-[11px] font-mono text-[#A1A1AA]">
                      ...
                      <span className="text-emerald-400 font-bold">AAA CONTRAST</span>
                    </div>
                    ...
                    <div className="grid grid-cols-4 gap-2">
                      {colorTokens.map((token) => (
                        ...
                        >
                          <div
                            className="w-full h-6 rounded-md border border-white/10 mb-1.5"
                            style={{ backgroundColor: token.hex }}
                          />
                          <span className="block font-mono text-[8px] text-[#A1A1AA] truncate">
                            {token.name}
                          </span>
                        </button>
                      ))}
                ...
                {activeStepIndex === 2 && (
                  <div className="p-5 rounded-2xl glass-subtle border border-white/10 space-y-4">
                    <div className="flex items-center justify-between text-[11px] font-mono text-[#A1A1AA]">
                      ...
                      <span className="text-[#F5F5F7] font-bold">BLUR 20px / SAT 190%</span>
                    </div>

                    {/* Interactive Glass Component */}
                    <div className="p-4 rounded-xl bg-white/6 border border-white/10 flex items-center justify-between shadow-xs">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-white flex items-center justify-center text-[#070709]">
                          <Maximize2 size={14} />
                        </div>
                        <div>
                          <span className="block text-xs font-bold text-[#F5F5F7]">
                      ...
                      <button
                        ...
                        className={`w-11 h-6 rounded-full p-0.5 transition-colors cursor-pointer ${
                          glassActiveToggle ? 'bg-emerald-400' : 'bg-white/20'
                        }`}
                      >
                        <div
                          className={`w-5 h-5 rounded-full bg-[#070709] transition-transform ${
                ...
                {activeStepIndex === 3 && (
                  <div className="p-5 rounded-2xl bg-[#121216] border border-white/10 text-white space-y-3">
                    ...
                    <div className="font-mono text-[11px] text-white/90 space-y-1 bg-[#18181F] p-3 rounded-lg border border-white/10 select-all">
                ...
                <div className="grid grid-cols-2 gap-3 pt-2">
                  {currentStep.hudMetrics.map((m) => (
                    <div key={m.label} className="p-3 rounded-xl glass-subtle border border-white/10">
                      <span className="block text-[10px] font-mono text-[#A1A1AA] uppercase">{m.label}</span>
                      <span className="block text-lg font-mono font-bold text-[#F5F5F7]">{m.val}</span>
                    </div>
                  ))}
                </div>
