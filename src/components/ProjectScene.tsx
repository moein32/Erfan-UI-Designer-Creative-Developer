...
export const ProjectScene: React.FC<ProjectSceneProps> = ({
  ...
  return (
    <div
      ...
      className="relative min-h-[95vh] md:min-h-screen py-16 md:py-28 flex items-center justify-center border-t border-white/10 overflow-hidden"
    >
      {/* Ambient background glow matching project accent color */}
      <div
        ref={bgAccentRef}
        className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[450px] md:w-[680px] h-[350px] md:h-[500px] rounded-full blur-[120px] pointer-events-none -z-10 opacity-30"
        style={{
          background: `radial-gradient(circle, ${project.accentColor}25 0%, ${project.secondaryAccent}15 60%, transparent 80%)`,
        }}
      />

      {/* Giant subtle watermark index number */}
      <div className="absolute right-12 top-1/2 -translate-y-1/2 text-[140px] sm:text-[200px] lg:text-[260px] font-bold opacity-[0.02] select-none pointer-events-none font-display leading-none z-0 text-white">
        0{index + 1}
      </div>
        ...
        >
          ...
          >
            ...
            <div className="flex items-center gap-3">
              <div className="glass-subtle px-3.5 py-1.5 rounded-full border border-white/10 flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-emerald-400"></span>
                <span className="text-[10px] font-bold uppercase tracking-[0.15em] text-[#F5F5F7]">
            ...
            <div className="space-y-2">
              <div className="flex flex-wrap items-baseline gap-4">
                <h3
                  className="font-display text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold tracking-tighter text-[#F5F5F7] cursor-pointer hover:text-white transition-colors"
                ...
                {project.persianTitle && (
                  <span className="text-base sm:text-lg font-persian text-[#A1A1AA]">
                    {project.persianTitle.split('—')[0]}
                  </span>
                )}
              </div>
              <p className="text-lg md:text-xl font-medium text-[#A1A1AA] leading-snug">
                {project.tagline}
              </p>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 pt-2">
              {project.metrics.slice(0, 3).map((metric) => (
                <div
                  key={metric.label}
                  className="p-3.5 rounded-2xl glass-subtle border border-white/10"
                >
                  <div
            ...
            <div className="space-y-2 pt-2">
              <span className="text-[10px] font-bold uppercase tracking-[0.15em] opacity-60 text-[#A1A1AA] block">
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-full text-xs font-mono transition-all flex items-center gap-1.5 cursor-pointer ${
                      activeScreenIndex === idx
                        ? 'bg-white text-[#070709] font-bold shadow-sm'
                        : 'glass-subtle border border-white/10 text-[#A1A1AA] hover:text-white hover:border-white/20'
                    }`}
                    onMouseEnter={() => setCursor({ type: 'button' })}
                    onMouseLeave={resetCursor}
                  >
                    {activeScreenIndex === idx && <Check size={11} className="text-emerald-500" />}
            ...
            <div className="flex flex-wrap items-center gap-4 pt-4">
              <button
                ...
                className="group px-6 py-3.5 rounded-full bg-white text-[#070709] text-xs font-mono font-bold tracking-wider hover:bg-[#E4E4E7] transition-all flex items-center gap-2 shadow-md active:scale-95 cursor-pointer"
                onMouseEnter={() => setCursor({ type: 'project', text: 'VIEW ↗' })}
                onMouseLeave={resetCursor}
              >
          ...
          >
            <div
              ...
              onMouseLeave={resetCursor}
            >
              {/* Subtle background card base in dark glass */}
              <div className="absolute inset-0 -m-6 sm:-m-10 rounded-3xl glass-medium border border-white/10 shadow-2xl backdrop-blur-md -z-10 group-hover:border-white/20 transition-all duration-500" />
